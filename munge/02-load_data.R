# Ce fichier sert à charger les données, les traiter et les mettre dans un
# fichier binaire

# Lire les fichiers et les fusionner en un seul data.frame
files <- paste0("./data/raw/", list.files("./data/raw/"))
apd <- ldply(files, read.csv)

# ATTENTION: Une alternative possible au code ci-dessus pourrait fonctionner
# mais génère une liste de dataframes plutôt qu'un seul

# files <- paste0("./data/raw/", list.files("./data/raw/")) # Ustilise recyclage
# apd <- Reduce(f = read.csv, files) # Donne une liste, dont il faut faire un data.frame

# fread importe un csv en data.table
# liste <- fread("./data/liste.fichiers.csv") 
...
#                 read.csv(
#                         file = x, header = T, fileEncoding = "Latin1"
#                 )
#         })
#         Reduce(function(x, y) {
#                 rbind(x, y)
...
noms.apd.nouv <-
        gsub(
                x = noms.apd.orig, pattern = "..", replacement = ".",
                fixed = T
        )
noms.apd.nouv <- gsub(
        x = noms.apd.nouv, pattern = "Organisation",
        replacement = "Org", fixed = F
)
noms.apd.nouv <- gsub(
        x = noms.apd.nouv, pattern = "marker.",
        replacement = "", fixed = T
)
noms.apd.nouv <-
        gsub(x = noms.apd.nouv, pattern = "[.]$", replacement = "")
...
apd <- apd %>% mutate(FY = as.numeric(str_sub(FY, start = -4))) %>%
        ...
        mutate(Country.region.percent = as.numeric(extract_numeric(Country.region.percent)) /
                       100) %>%
        mutate(Sector.percent = as.numeric(extract_numeric(Sector.percent)) /
                       100) %>%
...
pays.var <-
        c(
                "Continent.ID", "Continent.name", "Country.region.ID",
...
org <-
        org %>% separate(
                Org.type.location.profit.not.for.profit, c("Origine", "Type"), sep = " ", extra = "drop", fill = "warn"
        )
...
# On supprime cependant les phases de preparation de projet
proj <- proj[!str_detect(proj$No, "PRE")]

# On rajoute une année de début et une année de fin à chaque projet
duree <-
        proj %>% group_by(No) %>% summarise(debut = min(FY), fin = max(FY))
...
# Nettoyage
rm(list = c(
        "org.var", "pays.var", "proj.var", "sect.var", "temp", "liste", "multmerge"
))
